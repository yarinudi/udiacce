...
public class MainActivity extends AppCompatActivity {
    ...
    public static SensorEvent lastEvent;
    public static String events = "";
    private static Context c;
    private static SensorManager sm;
    private static Sensor accel;

    // Define a new listener for our accelerometer
    static class AccelListener implements SensorEventListener {

        public void onSensorChanged(SensorEvent event) {
            lastEvent = event;
            String values = Arrays.toString(lastEvent.values);
            values = values.replaceAll("\\s+","");
            events = events + values + " ";
            Log.d("MainActivity", "EVENT: " + values);
        }
    ...
    private void startPython(Map<String, File> paths) throws Exception {
        ...
        Python.run("udiacce", new String[0]);
    ...
    protected void onResume() {
        ...
        if (accel == null)
            ...
            Log.d(TAG, "onResume() complete");
            Log.d("MainActivity", "Listener is listening....");
        }
    }
    ...
    public static SensorEvent getLastEvent(){
        return lastEvent;
    }

    public static void deactivateListener(){
        sm.unregisterListener(accelListener, accel);
        Log.d("MainActivity", "Listener is sleeping....");
    }

    public static void activateListener(){
        if (accel == null)
            Log.d("MainActivity", "Can\'t activate listener");
        else{
            sm.registerListener(accelListener, accel, SensorManager.SENSOR_DELAY_NORMAL);
